...
fun AbrirChamadoScreen(
    ...
    val categoriaSelecionada = savedStateHandle?.getLiveData<com.example.nucleofornari.domain.model.chamado.TipoChamadoDto>("categoriaSelecionada")?.observeAsState()
    val descricao = remember { mutableStateOf("") }
    val criancaAtipica = remember { mutableStateOf(false) }
    var tipoChamado by remember { mutableStateOf(com.example.nucleofornari.domain.model.chamado.TipoChamadoDto()) }
